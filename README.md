...
It was meant to produce a [sine wave](https://en.wikipedia.org/wiki/Sine) but, even with the default parameters, it does not even come close to that.
After several tweaks to the function itself, I ended up using one I defined myself. This formula, albeit less fancy than the original, works as intended, while still including all the parameters that were featured in the provided one.
...
Due to economical, technical and safety restrictions, I was only able to test the Multi-machine case on my own: I used an old PC on which I mounted an [Ubuntu live session](https://ubuntu.com/tutorials/try-ubuntu-before-you-install#1-getting-started), and then proceeded pulling this repository on it. To the best of my knowledge, this testing setup does not feature any loss of generality with repsect to the one that was originally planned in the [V1.0 specifications](doc/Assignment%20Specifications%20V1.0.pdf).

Given these premises, results were satisfying nonetheless: my code works in both run modes, and does not seem to feature any relevant bug. The only known issues are highlighted by the **warnings** in this file, but they are mostly problems generated by users' errors, which are not always handled seamlessly.  
Going a bit deeper with the analysis, the Single-machine mode works flawlessly, while the Multi-machine one features one small annoyance: once the PC chain is broken, the computers that are still running keep registering tokens for a brief amount of time. These can be accurately discarded by reading the last token timestamp of the first PC that has been disconnected. Once that time value is collected from its log file, one can discard all the token values of the other log files that are past that time threshold. That said, it is usually a matter of less than 2 milliseconds worth of loggings.
