	...
	if (!run_mode)
	{
		while (1)
		{
			...
			else
			{
				...
				write(atoi(argv[3]), &(token), sizeof(token_struct)); //send received data on pipe 2; posso fare &msg e mandare tutto (vedi P)
				printf("G: Ho provato a scrivere sulla pipe\n"); //il problema è qui sopra: questa write non funziona!
			}
		}
	}
